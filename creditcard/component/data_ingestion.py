...
class DataIngestion:
    ...
    def download_creditcard_data(self,) -> str:
        try:
            ...
            file_path = os.path.join(download_dir,creditcard_file_name)
    ...
    def extract_file(self,file_path:str):
        try:
            ...
            with tarfile.open(file_path) as creditcard_file_obj:

                creditcard_file_obj.extractall(path=raw_data_dir)
    ...
    def split_data_as_train_test(self)-> DataIngestionArtifact:
        try:
            ...
            creditcard_data_frame["income_cat"] = pd.cut(
                creditcard_data_frame["median_income"],
            ...
            for train_index,test_index in split.split(creditcard_data_frame, creditcard_data_frame["income_cat"]):
                strat_train_set = creditcard_data_frame.loc[train_index].drop(["income_cat"],axis=1)
                strat_test_set= creditcard_data_frame.loc[test_index].drop(["income_cat"],axis=1)
    ...
    def initiate_data_ingestion(self)-> DataIngestionArtifact:
        try:
            file_path = self.download_creditcard_data()
